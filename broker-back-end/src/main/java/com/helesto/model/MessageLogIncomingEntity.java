...
@Entity
@Table(name = "messages_log_incoming")
@NamedNativeQueries({
    @NamedNativeQuery(
        name = "MessageLogIncoming.findAllBySenderCompID",
        query = 
                "SELECT * " +                                
                "FROM  messages_log_incoming " +
                "WHERE sendercompid = :sendercompid "  +
                "ORDER BY id DESC",
        resultClass = MessageLogIncomingEntity.class
    )
})
public class MessageLogIncomingEntity implements Serializable {
